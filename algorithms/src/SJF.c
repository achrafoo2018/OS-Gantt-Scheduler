...
void sjf(char config[]){
   ...
   for(int i=0; i < 3; i++){ 
      for(int j=2; j > i; j--){
         if(atoi(tab[j][1]) < atoi(tab[j-1][1]) || ((atoi(tab[j][1]) == atoi(tab[j-1][1])) && (atoi(tab[j][2]) < atoi(tab[j-1][2])))){
            for (int r = 0; r < 4; r++){
               char *t = tab[j - 1][r];
               tab[j - 1][r] = tab[j][r];
               tab[j][r] = t;
...
int t_a = atoi(tab[0][1]), t_e;
   for(int k=0; k < 3; k++){ 
      ...
      for(int q=k+1; q < 2; q++){
          if((atoi(tab[q][1]) <= finish && atoi(tab[q+1][1])<=finish) && (atoi(tab[q][2]) > atoi(tab[q+1][2]))){
